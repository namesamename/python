...
# 배경 만들기
bachground=pygame.image.load(os.path.join(image_path, 'background.png'))
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        #세로 위치(스테이지에 튕겨 올라가는 효과)
        if ball_pos_y>=screen_height-stage_height-ball_height:
            ball_val['to_y']=ball_val['init_spd_y']
        else: #그 외의 모든 경우에 속도 증가
            ball_val['to_y']+=0.5
